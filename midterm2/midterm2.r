...
mw1 <- infants[infants$gestation > 260,]
...
f1 <- family[(family$gender == "m") & (family$age > 50),]
...
for (k in 1:length(Cache500)){
	first.cache[k] <- Cache500[[k]][1]
}
...
mean.long.cache <- c() 
for (k in 1:length(Cache500)){
	if(length(Cache500[[k]]) > 51){
		mean.long.cache[k] <- mean(Cache500[[k]])
	}
	else {
		mean.long.cache[k] <- NA 
	}
